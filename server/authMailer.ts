...
function mailCopy(input: AuthChallengeEmail): { subject: string; text: string; html: string } {
  ...
  const action = input.type === 'email_verification' ? 'verify your Ultron account' : 'reset your Ultron password'
  const intro = input.type === 'email_verification'
    ? 'Use the verification code below to finish signing in to Ultron.'
    : 'Use the reset code below to choose a new Ultron password.'
  const expiry = input.expiresAt.toLocaleString()
  return {
    subject: input.type === 'email_verification' ? 'Verify your Ultron account' : 'Reset your Ultron password',
    text: [
      `Hi ${name},`,
      '',
      intro,
...
function organizationInviteMailCopy(input: OrganizationInviteEmail): { subject: string; text: string; html: string } {
  const inviter = input.invitedByDisplayName?.trim() || 'An Ultron workspace owner'
  const expiry = input.expiresAt.toLocaleString()
  const roleLabel = input.role === 'owner' ? 'workspace owner' : 'workspace member'
  return {
    subject: `Invitation to join ${input.organizationName} on Ultron`,
    text: [
      `Hi ${input.email},`,
      '',
      `${inviter} invited you to join ${input.organizationName} on Ultron as a ${roleLabel}.`,
      '',
      'Sign in to Ultron with this email address, then accept the invite from the app or use the invite token below.',
      '',
      `Invite token: ${input.inviteToken}`,
      `Open Ultron: ${input.acceptUrl}`,
      `Expires: ${expiry}`,
      '',
    ...
    html: [
      ...
      `<p>${escapeHtml(inviter)} invited you to join <strong>${escapeHtml(input.organizationName)}</strong> on Ultron as a ${escapeHtml(roleLabel)}.</p>`,
      ...
      `<div style="font-size:13px;color:#6b7280;margin-top:10px">Expires ${escapeHtml(expiry)}</div>`,
      '</div>',
      `<p><a href="${escapeHtml(input.acceptUrl)}" style="display:inline-block;padding:10px 16px;border-radius:999px;background:#111827;color:#ffffff;text-decoration:none;font-weight:600">Open Ultron</a></p>`,
