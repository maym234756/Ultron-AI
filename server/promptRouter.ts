export type RouterIntelligenceMode = 'instant' | 'balanced' | 'deep' | 'research'

export type RouterPreviewSettings = {
  intelligenceMode?: RouterIntelligenceMode
  autoRoute?: boolean
...
function normalizeIntelligence(value: unknown): RouterIntelligenceMode {
  return value === 'instant' || value === 'deep' || value === 'research' || value === 'balanced' ? value : 'balanced'
}

export function previewPromptRoute(
  text: string,
  hasFiles: boolean,
  hasImages: boolean,
  settings: RouterPreviewSettings,
  ...
  if (simpleKnowledge && !hasAgentAction) { chatScore += 3; pushSignal(signals, 'knowledge question') }
  if (shortPrompt && !hasFiles && !hasImages && !hasAgentAction) chatScore += 1
  if (hasConnector && !hasAccountContext && simpleKnowledge) chatScore += 2
  if (hasUrgency && !hasAgentAction && !hasResearchNeed) { chatScore += 1; pushSignal(signals, 'speed/briefness requested') }

  const useAgent = settings.autoRoute ?? true
    ? agentScore >= 3 && agentScore > chatScore
    : manualAgentMode

  let intelligenceMode = normalizeIntelligence(settings.intelligenceMode)
  let intelligenceReason = 'manual profile'
  if (settings.autoIntelligence ?? true) {
    ...
    } else if (INSTANT_ROUTE_RE.test(trimmed) || (simpleKnowledge && shortPrompt && !useAgent)) {
      intelligenceMode = 'instant'
